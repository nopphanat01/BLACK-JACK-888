...
struct Player {
    ...
    int highScore;   // คะแนนสูงสุด(เงินสูงสุด)
};
...
void updateStatistics(Player& player) {
    ...
    if (player.gamesPlayed > 0) {
        player.score = ((win*100)/(win+lose+Tie)); //(ชนะ/เกมที่เล่น)*100 
    }
}
...
void Calculatesplit(int scoreP , int scored){
    if (scoreP > 21) {
...
int playerdecision(vector<string>& deck, vector<string>& hand,vector<string>& dealerhand, int x, int& score, int& money, int scoredealer) {
    ...
    if (x == 1) {
        ...
        printCard(hand2);
        cout << "(" << score2 << ")\n";
        Calculatesplit(score1, scoredealer);
        cout << "Hand1\n";
        Calculatesplit(score2, scoredealer);
        cout << "Hand2\n";
        cout <<"-----------------------------------------------------------------------------\n";
    } else {
        cout << "(1) Hit  (2) Stand  ";
...
void printWithDelay(string text, int delay = 100) {
    for (size_t i = 0; i < text.length(); i++) {
        cout << text[i] << flush;  // พิมพ์ตัวอักษรทีละตัว
        this_thread::sleep_for(chrono::milliseconds(delay));  // ดีเลย์ก่อนพิมพ์ตัวถัดไป
    }
}

void showGameOverMessage(int playerMoney) {
    static bool shown2000 = false;
    static bool shown5000 = false;
    static bool shown10000 = false;

    if (playerMoney <= 0) {
        printWithDelay("\nYou're broke! Time to find a new hobby!\n\n", 50);
    } else if (playerMoney >= 2000 && !shown2000) {
        printWithDelay("\n2000? You're a high roller now! \n\n", 50);
        shown2000 = true; 
    } else if (playerMoney >= 5000 && !shown5000) {
        printWithDelay("\nBoom! There goes your 5000! That crazy \n\n", 50);
        shown5000 = true; 
    } else if (playerMoney >= 10000 && !shown10000) {
        printWithDelay("\nLevel up! +10000 gold obtained.\n\n", 50);
        shown10000 = true; 
    } else {
...
int main() {
    ...
    while (true)
    {
        printWithDelay("Welcome to Blackjack!\n", 50);
        printWithDelay("Would you like to (L)Load or (N)New game? : ", 50);
        cin >> ans;
        if(ans == "n" || ans == "N"){
            playerMoney = 1000;
            player.score = 0;
    ...
    while (playing&&playerMoney>0) {
        ...
        */
        ...
        showGameOverMessage(playerMoney);
